...
WELCOME_RU = (
    "Привет! Я отвечаю на вопросы по базе грантов "
    "(сейчас в ней {n} программ).\n\n"
    "Просто задай вопрос на русском или английском — например:\n"
    "• Какие гранты для биотеха в США?\n"
    "• Покажи дедлайны в мае 2026\n"
    "• Which grants are for AI startups in EU?\n\n"
    "Если ответа нет в базе, я честно скажу об этом, а не выдумаю."
)
...
HELP_RU = (
    ...
    "/start — приветствие\n"
    "/help — эта справка\n"
    "/count — сколько грантов в базе\n"
    "/reload — перечитать индекс (только для админа)\n\n"
    "По любому другому сообщению я ищу подходящие гранты в базе и отвечаю."
)
...
def handle_update(update: dict) -> list[Reply]:
    ...
    if text.startswith("/"):
        ...
        if cmd == "/count":
            return [Reply(chat_id, f"В базе сейчас {grant_count()} грантов.")]
        ...
        if cmd == "/reload":
            ...
            try:
                reload_index()
                return [Reply(chat_id, f"Индекс перечитан. Грантов: {grant_count()}.")]
            except Exception as exc:  # noqa: BLE001
                return [Reply(chat_id, f"Не удалось перезагрузить: {exc}")]

        return [Reply(chat_id, "Неизвестная команда. /help — список команд.")]
